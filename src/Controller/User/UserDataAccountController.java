        ...
        try{
            ...
            if (new_name.length() > 1 || new_lastname.length() > 1 || new_login.length() > 1 || new_password != null) {
                ...
                if(new_name.length() > 1){
                    DataBase.changeUserData(mainController.getCurrentUserId(), 1, new_name);
                    temp = 1;
                }
                if(new_lastname.length() > 1){
                    DataBase.changeUserData(mainController.getCurrentUserId(), 2, new_lastname);
                    temp = 2;
                }
                usernameThread.join();
                if(new_login.length() > 1){
                    if (usernameValidation.isValid) {
                        DataBase.changeUserData(mainController.getCurrentUserId(), 3, new_login);
                        temp = 3;
                    } else {
                ...
                if(new_password.length() > 1 || old_password.length() > 1){
                    if(passwordValidation.isCorrect){
                        ...
                        if (passwordValidation.isValid) {
                            DataBase.changeUserData(mainController.getCurrentUserId(), 4, new_password);
                            temp = 4;
                        } else {
